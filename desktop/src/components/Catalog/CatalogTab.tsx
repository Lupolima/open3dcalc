...
export function CatalogTab() {
  ...
  return (
    <div className="space-y-5">
      <div className="surface rounded-2xl p-5 flex flex-wrap gap-3 items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-[var(--color-text-primary)]">{t('catalog.title')}</h2>
          <p className="text-xs text-[var(--color-text-muted)]">{t('catalog.subtitle')}</p>
        </div>
        <div className="flex gap-2 text-xs">
          <span className="px-3 py-1.5 rounded-full bg-[var(--color-bg-elevated)] border border-[var(--color-border)] text-[var(--color-text-secondary)]">{stats.printers} {t('catalog.printers')}</span>
          <span className="px-3 py-1.5 rounded-full bg-[var(--color-bg-elevated)] border border-[var(--color-border)] text-[var(--color-text-secondary)]">{stats.materials} {t('catalog.materials')}</span>
          <span className="px-3 py-1.5 rounded-full bg-[var(--color-bg-elevated)] border border-[var(--color-border)] text-[var(--color-text-secondary)]">{stats.marketplaces} {t('catalog.marketplaces')}</span>
        </div>
      </div>

      <div className="surface rounded-2xl p-2 flex gap-2" role="tablist" aria-label={t('catalog.title')}>
        <button
          role="tab"
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[360px_1fr]">
      <div className="surface rounded-2xl p-5 space-y-3">
        <div className="text-sm font-semibold text-[var(--color-text-primary)]">{t('catalog.addPrinter')}</div>
        ...
        <div className="relative">
          ...
          <div className="relative flex justify-center text-xs text-[var(--color-text-muted)]"><span className="bg-[var(--color-bg-primary)] px-2">{t('catalog.orManual')}</span></div>
        </div>
      ...
      <div className="grid gap-3 md:grid-cols-2 2xl:grid-cols-3">
        {store.printers.map(p => (
          <div key={p.id} className="surface rounded-2xl p-4 space-y-2">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-2 min-w-0">
                <div className="min-w-0">
                  <div className="font-semibold text-[var(--color-text-primary)] truncate">{p.name}</div>
              ...
              {p.custom
                ? <span className="text-[10px] px-2 py-1 rounded-full bg-[var(--color-accent)]/20 text-[var(--color-accent)] shrink-0">Custom</span>
            ...
            <div className="text-xs text-[var(--color-text-secondary)]">{t('catalog.power')}: {p.power}W</div>
      ...
      {showEditModal && (
        ...
        >
          <div
            ref={modalRef}
            className="surface rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto"
            onClick={e => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-base font-bold text-[var(--color-text-primary)]">{t('catalog.editPrinter')}</h3>
              <button
                onClick={requestClose}
...
function MaterialManager() {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[360px_1fr]">
      <div className="surface rounded-2xl p-5 space-y-3">
        <div className="text-sm font-semibold text-[var(--color-text-primary)]">{t('catalog.addMaterial')}</div>
      ...
      <div className="grid gap-3 md:grid-cols-2 2xl:grid-cols-3">
        {store.materials.map(m => (
          <div key={m.id} className="surface rounded-2xl p-4 space-y-2">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="font-semibold text-[var(--color-text-primary)]">{m.name}</div>
                <div className="text-xs text-[var(--color-text-muted)] uppercase">{m.type}</div>
              </div>
              {m.custom && <span className="text-[10px] px-2 py-1 rounded-full bg-[var(--color-accent)]/20 text-[var(--color-accent)]">Custom</span>}
            </div>
            <div className="text-xs text-[var(--color-text-secondary)]">{t('catalog.density')}: {m.density}</div>
            <div className="text-xs text-[var(--color-text-secondary)]">{t('catalog.avgPrice')}: R$ {m.avgPrice}</div>
            {m.custom && <button onClick={() => store.removeMaterial(m.id)} className="text-xs text-[var(--color-danger)] hover:text-red-300 focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:outline-none rounded">{t('catalog.remove')}</button>}
          </div>
        ))}
      </div>
    </div>
...
function MarketplaceManager() {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[360px_1fr]">
      <div className="surface rounded-2xl p-5 space-y-3">
        <div className="text-sm font-semibold text-[var(--color-text-primary)]">{t('catalog.addMarketplace')}</div>
        ...
        <label className="flex items-center gap-2 text-sm text-[var(--color-text-secondary)] cursor-pointer">
          <input type="checkbox" checked={hasFreeShipping} onChange={e => setHasFreeShipping(e.target.checked)} className="rounded bg-[var(--color-bg-elevated)] border-[var(--color-border)]" />
      ...
      <div className="grid gap-3 md:grid-cols-2 2xl:grid-cols-3">
        {store.marketplaces.map(m => (
          <div key={m.id} className="surface rounded-2xl p-4 space-y-2">
            ...
            <div className="text-xs text-[var(--color-text-secondary)]">{t('catalog.fee')}: {m.feePercent}% + R$ {m.feeFixed}</div>
            <div className="text-xs text-[var(--color-text-secondary)]">{m.hasFreeShipping ? t('catalog.hasFreeShipping') : t('catalog.noFreeShipping')}</div>
            {m.custom && <button onClick={() => store.removeMarketplace(m.id)} className="text-xs text-[var(--color-danger)] hover:text-red-300 focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:outline-none rounded">{t('catalog.remove')}</button>}
          </div>
        ))}
      </div>
    </div>
