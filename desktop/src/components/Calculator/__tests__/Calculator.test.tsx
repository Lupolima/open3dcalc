...
vi.mock("@/components/ui/ToggleCard", () => ({
	ToggleSwitch: () => <div data-testid="toggle" />,
}));
vi.mock("@/hooks/useCurrency", () => ({
	useCurrency: () => ({ format: (v: number) => `R$${v}`, symbol: "R$" }),
}));
vi.mock("@/lib/printTimeEstimator", () => ({
	estimatePrintTimeFromDimensions: () => ({ estimatedHours: 1 }),
}));
vi.mock("@/components/Calculator/ResultsPanel", () => ({
	ResultsPanel: () => <div data-testid="results-panel" />,
}));
vi.mock("@/components/Calculator/sections/MaterialSection", () => ({
	MaterialSection: () => <div>MaterialSection</div>,
}));
vi.mock("@/components/Calculator/sections/PrintSection", () => ({
	PrintSection: () => <div>PrintSection</div>,
}));
vi.mock("@/components/Calculator/sections/FailureSection", () => ({
	FailureSection: () => <div>FailureSection</div>,
}));
vi.mock("@/components/Calculator/sections/MachineSection", () => ({
	MachineSection: () => <div>MachineSection</div>,
}));
vi.mock("@/components/Calculator/sections/FixedCostsSection", () => ({
...
vi.mock("@/stores/filamentInventory", () => ({
