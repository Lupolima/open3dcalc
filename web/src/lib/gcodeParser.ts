...
export function parseGcode(text: string): GcodeInfo {
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith(';MINX:')) {
      ...
      if (maxXLine) info.printSize.x = parseFloat(maxXLine.trim().split(':')[1]) - minX
      if (minYLine && maxYLine) info.printSize.y = parseFloat(maxYLine.trim().split(':')[1]) - parseFloat(minYLine.trim().split(':')[1])
      if (minZLine && maxZLine) info.printSize.z = parseFloat(maxZLine.trim().split(':')[1]) - parseFloat(minZLine.trim().split(':')[1])
    }
    ...
    if (trimmed.startsWith('; layer_height')) {
      info.layerHeight = parseFloat(trimmed.split('=')[1] || trimmed.split(':')[1])
    }
    if (trimmed.startsWith('; initial_layer_line_height')) {
      if (!info.layerHeight) info.layerHeight = parseFloat(trimmed.split('=')[1] || trimmed.split(':')[1])
    }
    if (trimmed.startsWith('; nozzle_temperature')) {
      info.nozzleTemp = parseFloat(trimmed.split('=')[1] || trimmed.split(':')[1])
    }
    ...
    if (trimmed.startsWith('M104') || trimmed.startsWith('M109')) {
      ...
      if (tempMatch && !info.nozzleTemp) info.nozzleTemp = parseInt(tempMatch[1])
    }
  ...
  if (info.filamentUsedMm > 0) {
    ...
    const volumeCm3 = Math.PI * Math.pow(radiusMm / 10, 2) * (info.filamentUsedMm / 10)
