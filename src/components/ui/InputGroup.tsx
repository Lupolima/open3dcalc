  ...
  return (
    <div className={`flex flex-col gap-1 ${className}`}>
      <div className="flex items-center gap-1.5">
        <label className={`text-[11px] font-medium ${error ? 'text-red-400' : 'text-gray-400'}`}>{label}</label>
        {tooltip && (
      ...
      <div className="relative flex items-center">
        ...
        <input
          ...
          step={step}
          className={`w-full bg-white/5 border rounded-xl text-sm text-white h-10 px-3 focus:ring-2 outline-none transition-all placeholder:text-gray-500 ${
            prefix ? 'pl-9' : ''
          } ${unit ? 'pr-12' : ''} ${
            error
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <select
        value={value}
        onChange={e => onChange(e.target.value)}
        className="w-full bg-white/5 border border-white/10 rounded-xl text-sm text-white h-10 px-3 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition-all appearance-none"
      >
        {options.map(opt => (
