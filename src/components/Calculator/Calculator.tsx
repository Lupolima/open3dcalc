...
export function Calculator() {
  ...
  const [materialToggled, setMaterialToggled] = useState(true)
  const [printToggled, setPrintToggled] = useState(true)
  const [machineToggled, setMachineToggled] = useState(store.fdmMachine.enabled)
  const [hardwareToggled, setHardwareToggled] = useState(true)
  const [laborToggled, setLaborToggled] = useState(store.fdmLabor.enabled)
  const [salesToggled, setSalesToggled] = useState(true)
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <ToggleCard title={t('calc.printParams')} icon="⚙️" enabled={printToggled} onToggle={setPrintToggled}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {isFDM && (
            <ToggleCard title={t('calc.fdmHardware')} icon="🔧" enabled={hardwareToggled} onToggle={setHardwareToggled} theme="sky">
          ...
          {isFDM && (
            <ToggleCard title={t('calc.fdmFinishing')} icon="🎨" enabled={store.fdmFinishing.enabled}
              onToggle={v => store.setFdmFinishing({ ...store.fdmFinishing, enabled: v })} theme="sky">
          ...
          {!isFDM && (
            <ToggleCard title={t('calc.resinPostProcess')} icon="🧪"
              enabled={store.resinPostProcess.washingEnabled || store.resinPostProcess.curingEnabled}
              onToggle={v => store.setResinPostProcess({ ...store.resinPostProcess, washingEnabled: v, curingEnabled: v })}>
              <div className="space-y-4">
                <div className="flex items-center justify-between">
          ...
          {!isFDM && (
            <ToggleCard title={t('calc.resinHardware')} icon="🖥️" enabled={store.resinHardware.enabled}
              onToggle={v => store.setResinHardware({ ...store.resinHardware, enabled: v })}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <ToggleCard title={t('calc.machine')} icon="🖨️" enabled={machineToggled} onToggle={setMachineToggled}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <ToggleCard title={t('calc.labor')} icon="👷" enabled={laborToggled} onToggle={setLaborToggled}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <ToggleCard title={t('calc.opsSoftware')} icon="🛡️"
            enabled={(isFDM ? store.fdmOps.enabled : store.resinOps.enabled) || (isFDM ? store.fdmSoft.enabled : store.resinSoft.enabled)}
            onToggle={v => {
              if (isFDM) { store.setFdmOps({ ...store.fdmOps, enabled: v }); store.setFdmSoft({ ...store.fdmSoft, enabled: v }) }
              else { store.setResinOps({ ...store.resinOps, enabled: v }); store.setResinSoft({ ...store.resinSoft, enabled: v }) }
            }}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
          ...
          <ToggleCard title={t('calc.sales')} icon="💰" enabled={salesToggled} onToggle={setSalesToggled}>
            <div className="space-y-4">
