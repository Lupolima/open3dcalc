...
import { calculateInfillImpact } from '@/shared/lib/calculator'
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      <div className="surface rounded-xl p-6 sm:p-7">
        <h2 className="text-lg sm:text-xl font-bold text-[var(--color-text-primary)]">{t('calc.infillPercent')} Calculator</h2>
        <p className="text-xs sm:text-sm text-[var(--color-text-muted)] mt-1">Veja como o % de infill afeta peso, custo e tempo</p>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(320px,380px)_minmax(0,1fr)]">
        <div className="surface rounded-xl p-6 sm:p-7 space-y-4">
          <div className="text-sm sm:text-[15px] font-semibold text-[var(--color-text-primary)]">Dimensões (mm)</div>
        ...
        <div className="space-y-4 sm:space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 sm:gap-3">
            <div className="surface rounded-xl p-4 sm:p-5 text-center">
              <p className="text-[10px] sm:text-xs text-[var(--color-text-secondary)] mb-1">Peso Estimado</p>
              <p className="text-lg sm:text-xl font-bold text-[var(--color-info)]">{result.weight.toFixed(1)}g</p>
            </div>
            <div className="surface rounded-xl p-4 sm:p-5 text-center">
              <p className="text-[10px] sm:text-xs text-[var(--color-text-secondary)] mb-1">Custo Material</p>
              <p className="text-lg sm:text-xl font-bold text-[var(--color-success)]">{fmtCurrency(result.cost)}</p>
            </div>
            <div className="surface rounded-xl p-4 sm:p-5 text-center">
              <p className="text-[10px] sm:text-xs text-[var(--color-text-secondary)] mb-1">Volume Sólido</p>
          ...
          {/* Comparison Table */}
          <div className="surface rounded-xl p-6 sm:p-7">
            <h3 className="text-sm sm:text-[15px] font-bold text-[var(--color-text-primary)] mb-3">Comparação de Infill</h3>
              ...
              <table className="w-full text-xs sm:text-sm">
                ...
                <tbody>
                  {comparisons.map(c => {
                    ...
                    return (
                      <tr key={c.infill} className={`border-b border-[var(--color-border)] ${c.infill === parseFloat(infillPercent) ? 'bg-purple-600/10' : ''}`}>
                        ...
                        <td className={`text-right py-2 px-3 ${diff > 0 ? 'text-[var(--color-danger)]' : 'text-[var(--color-text-muted)]'}`}>
