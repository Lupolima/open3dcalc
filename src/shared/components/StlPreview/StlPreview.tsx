...
export function StlPreview({ geometry }: StlPreviewProps) {
  ...
  if (!geometry) {
    return (
      <div className="surface rounded-xl h-48 flex items-center justify-center">
        <p className="text-sm text-[var(--color-text-secondary)]">{t('stl.noModel')}</p>
      </div>
    )
  }

  return (
    <div className="surface rounded-xl h-48 overflow-hidden">
